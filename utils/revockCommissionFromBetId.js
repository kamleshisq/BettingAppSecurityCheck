        ...
        if(allCommissionFromThatBEt.length > 0){
            for(let i = 0; i < allCommissionFromThatBEt.length; i++){
                if(allCommissionFromThatBEt[i].commissionStatus != 'Unclaimed' &&  allCommissionFromThatBEt[i].commissionStatus != 'cancel'){
                    let user = await User.findByIdAndUpdate(allCommissionFromThatBEt[i].userId, {$inc:{availableBalance: -allCommissionFromThatBEt[i].commission, uplinePL:allCommissionFromThatBEt[i].commission, myPL:-allCommissionFromThatBEt[i].commission, pointsWL:-allCommissionFromThatBEt[i].commission}})
                    if(user){
