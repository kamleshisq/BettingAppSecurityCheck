...
if(!marketDetails.runners){
    ...
    if(commissionMarket.some(item => item.marketId == data.data.market)){
        ...
        let commissionPer = 0
        if ((marketDetails.title.startsWith('Bookmake')|| bet.marketName.startsWith('BOOKMAKE') || marketDetails.title.startsWith('TOSS') ||  marketDetails.title.startsWith('BOOK')) && commission[0].Bookmaker.type == "ENTRY" && commission[0].Bookmaker.status){
