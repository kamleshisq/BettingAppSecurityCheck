        ...
        try{
            for(const bets in allBetWithMarketId){
                if(allBetWithMarketId[bets].status === 'WON'){
                    ...
                    let user = await User.findByIdAndUpdate(allBetWithMarketId[bets].userId, {$inc:{balance: -VoidAmount, availableBalance: -VoidAmount, myPL: -VoidAmount, exposure: allBetWithMarketId[bets].Stake.toFixed(2)}})
