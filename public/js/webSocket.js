...
socket.on('connect', () => {
    ...
    socket.on('loginUser',(data) => {
        ...
        //       if (styleSheet.href) {
        //         if(styleSheet.href == "https://dev.ollscores.com/assets/css/style.css" || styleSheet.href == "https://dev.ollscores.com/assets/css/loggedin-page-style.css" || styleSheet.href =="https://dev.ollscores.com/assets/css/media.css"){
        ...
        //172.105.58.243:8880 172.105.58.243:8880 https://dev.ollscores.com/admin/userManagement https://dev.ollscores.com  http: 
  ...
  socket.on('UserUpdatePass', async(data)=>{
       ...
       // Add a common class to the containers that wrap each section of the HTML
        ...
        $(document).ready(function () {
            ...
            if(data.secId.startsWith('odd_Even_')){
                ...
                specificSpan = $(`#${secforFency}`).children("span").eq(0).text();
                // console.log(`#${secforFency}`)
            }else{
    ...
    if(pathname == "/myCommissionReportsMatch"){
        ...
        socket.on('getDetailsCommision', data => {
            ...
            $('#commisiionDetailsData').html(html)
        })
    }
        ...
        socket.on('getsportwiseuplinecommission',async(data)=>{
            ...
            if(data.status == 'success'){
              ...
              if(result.length > 0){
                ...
                html2 += `  <li class="active sportwise" data-sportwise='${parentdata.sportId}'>${data.bredcum[0]}</li>`
