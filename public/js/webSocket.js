    ...
    socket.on('getAccountsData', async(data) => {
        ...
        if(data.status === 'err'){
            ...
            if(data.data.length > 0){
                ...
                let newHTML =`<p>Didn't find the account? Click  <a class="payment-mng-link" href="/manageAccounts" >here</a> to add new</p>`
                $('.luck-enterprise ul#BANK-DATA1').after(newHTML);
            }else{
                $('#navmod4').find('#enter-withdraw-detail').addClass('hide-elemnt')
                html = `<p>Didn't find the account? Click  <a class="payment-mng-link" href="/manageAccounts" >here</a> to add new</p>`
