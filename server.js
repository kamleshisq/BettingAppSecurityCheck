...
//......................FOR user management page .......................//
    ...
    socket.on('settlement',async(data)=>{
        ...
        if(data.fromdate && data.todate){
            dataobj = {$gte:new Date(data.fromdate) ,$lte:new Date(data.todate)}
        }else if(data.fromdate && !data.todate){
            dataobj = {$gte:new Date(data.fromdate)}
        }else if(!data.fromdate && data.todate){
            dataobj = {$lte:new Date(data.todate)}
        }
        // console.log(dataobj)
    ...
    socket.on("VoidBetIn", async(data) => {
        let checkPassOfUser =  await checkPass(data.LOGINDATA.LOGINUSER, data.data.password)
        console.log(checkPassOfUser)
        // try{
